...
import {
  volatilityTraderModes,
  volatilityTraderScenarios,
  type VolatilityTraderChoice,
  type VolatilityTraderMode,
  type VolatilityTraderScenario,
} from './volatilityTraderScenarios';

type AttemptState = { choice?: VolatilityTraderChoice; revealed?: boolean };

const STORAGE_KEY = 'market-world-model:2.14-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<VolatilityTraderChoice>(['a', 'b', 'c']);
const scenarioIds = new Set(volatilityTraderScenarios.map((scenario) => scenario.id));
const sequence = volatilityTraderScenarios;
...
function validatedAttempts(value: unknown): Record<string, AttemptState> | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    if (!validChoices.has(attempt.choice as VolatilityTraderChoice) || typeof attempt.revealed !== 'boolean') return;
    result[id] = { choice: attempt.choice as VolatilityTraderChoice, revealed: attempt.revealed };
  });
  return result;
}

function taskStatus(scenario: VolatilityTraderScenario, attempt?: AttemptState) {
  if (!attempt?.revealed) return '未提交';
  return attempt.choice === scenario.correct ? '已答对' : '已提交，待重做';
}

function ScenarioFacts({ scenario }: { scenario: VolatilityTraderScenario }) {
  return (
...
export default function VolatilityTraderLab() {
  const [mode, setMode] = useState<VolatilityTraderMode>('object');
  ...
  const scenarios = volatilityTraderScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: VolatilityTraderMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function selectMode(nextMode: VolatilityTraderMode) {
  ...
  function choose(nextChoice: VolatilityTraderChoice) {
  ...
  function goToNextUnsubmitted() {
    if (nextUnsubmitted) {
      const nextModeScenarios = volatilityTraderScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  function resetAll() {
    ...
    navigate('object', 0, 'option');
  }

  const globalQuestionNumber = sequence.findIndex((item) => item.id === scenario.id) + 1;

  return (
    <div aria-busy={restoring} className="impact-lab bank-intermediary-lab volatility-trader-lab">
      <div className="impact-lab-head">
        <div><span>VOLATILITY-TRADER BOOK LAB</span><h3>从波动对象、隐含价格与路径损益到可执行组合</h3></div>
        <p>十题使用冻结教学参数。先分开 volatility、variance、realized path、option 与 event total variance，再审计 variance swap、calendar、dispersion、target–fill 和 short-vol constraints；客户端包含答案，不作为防作弊考试。</p>
      </div>
      <noscript>
      ...
      <div className="impact-mode-picker" role="group" aria-label="实验模式">
        {volatilityTraderModes.map((item) => (
      ...
      <div className="impact-task-picker" role="group" aria-label={(mode === 'object' ? '识别真正的交易对象' : '审计组合、执行与约束') + '题目导航'}>
      ...
      {completed ? (
        <div className="impact-result" ref={resultRef} role="region" tabIndex={-1} aria-labelledby={resultTitleId}>
          ...
          <p>复习路径：第 1–2 题锁定 volatility／variance 与 realized-measure 单位；第 3–5 题检查 option 不是单一 vol、局部 P&amp;L 和 event total variance；第 6–8 题进入 variance swap、calendar 与 dispersion；第 9 题分开 target、候选订单和 fill；第 10 题用 tail、margin 与 liquidity 约束 short-vol 规模。错误题保留原选择，可逐题重做。</p>
          <div>
            <button className="impact-secondary" onClick={() => navigate('object', 0)} type="button">回到第 1 题</button>
            ...
            {scenario.options.map((option, optionIndex) => (
              <label className={choice === option.id ? 'selected' : ''} key={option.id}>
                <input
                  ...
                  name={'volatility-trader-' + scenario.id}
      ...
      <p className="impact-lab-caveat"><b>模型边界：</b>所有 returns、volatility、variance、Greeks、option terms、notional、correlation、target、margin 和成交均为得到唯一答案而冻结的教学参数，不代表任何产品、账户或市场的当前状态。局部 Greek 展开不替代 full repricing；题目不是投资建议、收益保证或真实交易；保存只限本设备、本浏览器和当前题库版本。</p>
    </div>
  );
}
