...
type AttemptState = { choice?: RedemptionChoice; revealed?: boolean };

const STORAGE_KEY = 'market-world-model:2.17-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<RedemptionChoice>(['a', 'b', 'c']);
const scenarioIds = new Set(redemptionScenarios.map((scenario) => scenario.id));
const sequence = redemptionScenarios;
...
function validatedAttempts(value: unknown): Record<string, AttemptState> | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    if (!validChoices.has(attempt.choice as RedemptionChoice) || typeof attempt.revealed !== 'boolean') return;
    result[id] = { choice: attempt.choice as RedemptionChoice, revealed: attempt.revealed };
  });
  return result;
}

function taskStatus(scenario: RedemptionScenario, attempt?: AttemptState) {
  if (!attempt?.revealed) return '未提交';
  return attempt.choice === scenario.correct ? '已答对' : '已提交，待重做';
}

function ScenarioFacts({ scenario }: { scenario: RedemptionScenario }) {
  return (
...
export default function RedemptionLab() {
  const [mode, setMode] = useState<RedemptionMode>('ledger');
  ...
  const scenarios = redemptionScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: RedemptionMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(nextChoice: RedemptionChoice) {
  ...
  function goToNextUnsubmitted() {
    if (nextUnsubmitted) {
      const modeScenarios = redemptionScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  return (
    <div aria-busy={restoring} className="impact-lab redemption-lab">
      <div className="impact-lab-head">
        <div><span>REDEMPTION LEDGER &amp; EVIDENCE LAB</span><h3 ref={labTitleRef} tabIndex={-1}>从份额退出到可识别的市场成交</h3></div>
        <p>十题都冻结币种、份额、时钟和成本口径。前五题重建 gross/net、cash gap、sale target、NAV 与 buffer；后五题区分成本分配、先动激励、gate、fire-sale 证据和 ETF/AP 边界。客户端包含答案，不作为防作弊考试。</p>
      </div>
      <noscript>
        <div className="precision-note"><span>互动实验当前不可用</span><p>此实验需要浏览器运行 JavaScript；你仍可前往 <a href="#active-practice">主动练习</a> 与 <a href="#checks-interfaces">理解检查</a>，使用同一数据源生成的静态题和完整答案。</p></div>
      </noscript>
      ...
      <div className="impact-task-picker" role="group" aria-label={(mode === 'ledger' ? '赎回账本' : '机制推断') + '题目导航'}>
      ...
      {completed ? (
        <div className="impact-result" ref={resultRef} role="region" tabIndex={-1} aria-labelledby={resultTitleId}>
          ...
          <p>复习路径：第1题保存 gross 与 net；第2题坚持 payment deadline；第3题分开 target/order/fill/settlement；第4题核对成本账本；第5题分开最低缺口与 buffer；第6–8题识别工具改变的状态；第9题使用证据阶梯；第10题回到 ETF/AP 边界。</p>
          <div>
            ...
            {!resetArmed ? <button className="impact-secondary" onClick={requestReset} type="button">准备重置全部作答</button> : <>
              ...
              <button className="impact-secondary" onClick={() => { setResetArmed(false); setAnnouncement('已取消重置，原作答记录保留。'); }} type="button">取消重置</button>
            </>}
          </div>
        </div>
      ) : (
          ...
          <fieldset className="impact-choice-fieldset" disabled={restoring || revealed}>
            ...
            {scenario.options.map((option, optionIndex) => (
              <label className={choice === option.id ? 'selected' : ''} key={option.id}>
                <input checked={choice === option.id} name={'redemption-' + scenario.id} onChange={() => choose(option.id)} ref={optionIndex === 0 ? firstOptionRef : undefined} type="radio" value={option.id} />
      ...
      <p className="impact-lab-caveat"><b>模型与制度边界：</b>所有金额、时钟、gate、fees、orders 与 fills 都是为唯一答案冻结的教学参数，不代表任何基金或法域的现行条款。题目不是投资、监管或法律建议；记录只保存在本设备、本浏览器和当前题库版本，不会上传，清理浏览器数据会丢失。</p>
    </div>
  );
}
