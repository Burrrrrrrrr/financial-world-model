...
import {
  expectationsModes,
  inflationExpectationsScenarios,
  type ExpectationsChoice,
  type ExpectationsMode,
  type ExpectationsScenario,
} from './inflationExpectationsScenarios';

type Confidence = 'low' | 'medium' | 'high';
type AttemptState = {
  choice?: ExpectationsChoice;
  confidence?: Confidence;
  firstChoice?: ExpectationsChoice;
  firstConfidence?: Confidence;
  submissions?: number;
  revealed: boolean;
};

// Editorial revisions may keep this key only while schema 1 and the ten scenario IDs remain compatible.
const STORAGE_KEY = 'market-world-model:3.04-lab-r1';
const EMPTY_ATTEMPT: AttemptState = { revealed: false };
const validChoices = new Set<ExpectationsChoice>(['a', 'b', 'c']);
const validConfidence = new Set<Confidence>(['low', 'medium', 'high']);
const scenarioIds = new Set(inflationExpectationsScenarios.map((scenario) => scenario.id));
const sequence = inflationExpectationsScenarios;
...
function validateStored(value: unknown): { attempts: Record<string, AttemptState>; droppedInvalid: boolean } | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    const choice = item.choice as ExpectationsChoice | undefined;
    const confidence = item.confidence as Confidence | undefined;
    const firstChoice = item.firstChoice as ExpectationsChoice | undefined;
    const firstConfidence = item.firstConfidence as Confidence | undefined;
    const submissions = item.submissions;
    // Early r1 drafts omitted this field for an unsubmitted selection. Normalize that record as unrevealed.
    const revealed = item.revealed === undefined ? false : item.revealed;
    const currentInvalid = revealed === true
      ? !validChoices.has(choice as ExpectationsChoice) || !validConfidence.has(confidence as Confidence)
    ...
    const completeHistory = validChoices.has(firstChoice as ExpectationsChoice)
...
function statusOf(scenario: ExpectationsScenario, attempt?: AttemptState) {
...
export default function InflationExpectationsLab() {
  const [mode, setMode] = useState<ExpectationsMode>('measurement');
  ...
  const scenarios = inflationExpectationsScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: ExpectationsMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(choice: ExpectationsChoice) {
    setAttempts((current) => {
  ...
  function goToNext() {
    ...
    const nextModeScenarios = inflationExpectationsScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  return (
    <div aria-busy={loading} className="impact-lab inflation-expectations-lab">
      <div className="impact-lab-head">
        <div>
          <span>EXPECTATIONS · OBJECT-TO-ACTION LAB</span>
          <h3 ref={titleRef} tabIndex={-1}>先把对象、期限与分布量对，再把更新、锚定、合同和实时资产新闻接成一条可检验链</h3>
        </div>
        <p>前五题校准 SCE 期限、uncertainty／disagreement、vintage、breakeven 与三维锚定；后五题复算 Bayesian、sticky information、Fisher、反馈稳定性与事件窗。</p>
      </div>

      <noscript><div className="precision-note"><span>互动实验当前不可用</span><p>此实验需要 JavaScript；你仍可前往<a href="#expectations-static-twins">十道无脚本静态孪生题</a>完成同一组机制练习。</p></div></noscript>
      ...
      {completed ? (
        <div aria-label="实验完成诊断" className="impact-result correct" ref={resultRef} role="region" tabIndex={-1}>
          ...
          <p>完成不等于掌握。优先重做高置信却答错的题，再用静态孪生检验自己是否守住对象、期限、分布、数据时钟、行为断点和市场楔子。</p>
          <button className="impact-secondary" onClick={() => navigate('measurement', 0)} type="button">回到第一题逐题复盘</button>
        </div>
      ) : (
        <>
          <section className="impact-question" aria-labelledby={`expectations-question-${scenario.id}`}>
            <span>{scenario.label} · 全局第 {globalNumber}/10 题</span>
            <h3 id={`expectations-question-${scenario.id}`} ref={questionRef} tabIndex={-1}>{scenario.title}</h3>
            <p>{scenario.brief}</p>
          </section>
            ...
            {scenario.options.map((option, index) => (
              <label className={attempt.choice === option.id ? 'selected' : ''} key={option.id}>
                <input
                  ...
                  name={`expectations-choice-${scenario.id}`}
          ...
          <fieldset className="impact-choice-fieldset" disabled={loading || attempt.revealed}>
            ...
            {confidenceLabels.map((item) => (
              <label key={item.id}>
                <input checked={attempt.confidence === item.id} name={`expectations-confidence-${scenario.id}`} onChange={() => chooseConfidence(item.id)} type="radio" value={item.id} />
      ...
      <p className="impact-lab-caveat"><b>边界：</b>题中期限、分布、风险溢价、更新参数、反馈系数和事件窗只为唯一判分而冻结；它们不是通胀预测、政策建议或投资建议。记录仅保存在本设备、本浏览器和当前题库版本，不会上传。</p>
    </div>
  );
}
