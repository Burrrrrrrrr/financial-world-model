...
import {
  globalMacroModes,
  globalMacroScenarios,
  type GlobalMacroChoice,
  type GlobalMacroMode,
  type GlobalMacroScenario,
} from './globalMacroScenarios';

type AttemptState = { choice?: GlobalMacroChoice; revealed?: boolean };

const STORAGE_KEY = 'market-world-model:2.13-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<GlobalMacroChoice>(['a', 'b', 'c']);
const scenarioIds = new Set(globalMacroScenarios.map((scenario) => scenario.id));
const sequence = globalMacroScenarios;
...
function validatedAttempts(value: unknown): Record<string, AttemptState> | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    if (!validChoices.has(attempt.choice as GlobalMacroChoice) || typeof attempt.revealed !== 'boolean') return;
    result[id] = { choice: attempt.choice as GlobalMacroChoice, revealed: attempt.revealed };
  });
  return result;
}

function taskStatus(scenario: GlobalMacroScenario, attempt?: AttemptState) {
  if (!attempt?.revealed) return '未提交';
  return attempt.choice === scenario.correct ? '已答对' : '已提交，待重做';
}

function ScenarioFacts({ scenario }: { scenario: GlobalMacroScenario }) {
  return (
...
export default function GlobalMacroLab() {
  const [mode, setMode] = useState<GlobalMacroMode>('translate');
  ...
  const scenarios = globalMacroScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: GlobalMacroMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function selectMode(nextMode: GlobalMacroMode) {
  ...
  function choose(nextChoice: GlobalMacroChoice) {
  ...
  function goToNextUnsubmitted() {
    if (nextUnsubmitted) {
      const nextModeScenarios = globalMacroScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  function resetAll() {
    ...
    navigate('translate', 0, 'option');
  }

  const globalQuestionNumber = sequence.findIndex((item) => item.id === scenario.id) + 1;

  return (
    <div aria-busy={restoring} className="impact-lab bank-intermediary-lab global-macro-lab">
      <div className="impact-lab-head">
        <div><span>GLOBAL-MACRO TRANSLATION LAB</span><h3>从相对预期的分歧到可核算暴露与证据边界</h3></div>
        <p>十题使用冻结教学参数。先区分数据水平、surprise 和价格隐含量，再核算 DV01、曲线、FX、carry、股指、商品、gross/net 与候选订单；客户端包含答案，不作为防作弊考试。</p>
      </div>
      <noscript>
      ...
      <div className="impact-task-picker" role="group" aria-label={(mode === 'translate' ? '把观点翻译成暴露' : '审计 carry、约束与证据') + '题目导航'}>
      ...
      {completed ? (
        <div className="impact-result" ref={resultRef} role="region" tabIndex={-1} aria-labelledby={resultTitleId}>
          ...
          <p>复习路径：第 1–2 题分开 data、surprise、survey 与 price-implied baseline；第 3–5 题把利率、曲线和 FX 观点写成带单位的情景 P&amp;L；第 6–8 题审计 carry、股指与商品期限价格；第 9 题分开 gross、net、margin 与现金，第 10 题检验为什么 target 还不能识别真实 flow。错误题保留原选择，可逐题重做。</p>
          <div>
            <button className="impact-secondary" onClick={() => navigate('translate', 0)} type="button">回到第 1 题</button>
      ...
      <p className="impact-lab-caveat"><b>模型边界：</b>所有宏观公布、调查、价格隐含量、利率、DV01、合约、notional、carry、basis、便利收益、现金与保证金都是为了唯一答案而冻结的教学参数，不代表任何基金、国家、产品或市场的当前状态。题目不是投资建议、收益保证或真实交易；保存只限本设备、本浏览器和当前题库版本。</p>
    </div>
  );
}
