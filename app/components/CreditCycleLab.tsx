...
import {
  ...
  type CreditCycleChoice,
  type CreditCycleMode,
  type CreditCycleScenario,
} from './creditCycleScenarios';
import styles from './creditCycle.module.css';

type Confidence = 'low' | 'medium' | 'high';
type AttemptState = {
  choice?: CreditCycleChoice;
  confidence?: Confidence;
  firstChoice?: CreditCycleChoice;
  firstConfidence?: Confidence;
  submissions?: number;
  revealed?: boolean;
};

const STORAGE_KEY = 'market-world-model:3.14-credit-cycle-lab-r1';
const STORAGE_SCHEMA = 3;
const EMPTY_ATTEMPT: AttemptState = {};
const DEFAULT_MODE: CreditCycleMode = 'calculation';
const choiceIds: CreditCycleChoice[] = ['a', 'b', 'c'];
const confidenceLabels = [
  { id: 'low' as const, label: '低：我在排除明显错误' },
  { id: 'medium' as const, label: '中：我理解主要机制' },
  { id: 'high' as const, label: '高：我能解释识别边界' },
];

function isChoice(value: unknown): value is CreditCycleChoice {
  return typeof value === 'string' && choiceIds.includes(value as CreditCycleChoice);
}
...
function parseAttempt(value: unknown): AttemptState | null {
  ...
  const allowedKeys = new Set(['choice', 'confidence', 'firstChoice', 'firstConfidence', 'submissions', 'revealed']);
  ...
  if (submissions === 0 && hasFirstPair) return null;
  if (revealed && (submissions < 1 || candidate.choice === undefined || candidate.confidence === undefined || !hasFirstPair)) return null;
  return {
    choice: candidate.choice as CreditCycleChoice | undefined,
    confidence: candidate.confidence as Confidence | undefined,
    firstChoice: candidate.firstChoice as CreditCycleChoice | undefined,
...
function parseStored(raw: string): { attempts: Record<string, AttemptState>; droppedInvalid: boolean } | null {
  try {
    ...
    const legalIds = new Set<string>(creditCycleScenarios.map(({ id }) => id));
...
export const creditCycleStorageAudit = [
  { key: 'empty and pre-submit partial states remain recoverable', passed: parseAttempt({}) !== null && parseAttempt({ choice: 'a' }) !== null },
  { key: 'revealed state requires choice confidence first-attempt pair and positive submissions', passed: parseAttempt({ choice: 'a', confidence: 'high', firstChoice: 'a', firstConfidence: 'high', submissions: 1, revealed: true }) !== null && parseAttempt({ choice: 'a', confidence: 'high', revealed: true }) === null },
  { key: 'retry state preserves a coherent first-attempt pair', passed: parseAttempt({ firstChoice: 'b', firstConfidence: 'low', submissions: 1, revealed: false }) !== null && parseAttempt({ firstChoice: 'b', submissions: 1, revealed: false }) === null },
  { key: 'unknown fields and first-attempt metadata without a submission are rejected', passed: parseAttempt({ injected: true }) === null && parseAttempt({ firstChoice: 'a', firstConfidence: 'medium', submissions: 0 }) === null },
] as const;

if (!creditCycleStorageAudit.every(({ passed }) => passed)) {
  throw new Error(`3.14 storage schema gate failed: ${creditCycleStorageAudit.filter(({ passed }) => !passed).map(({ key }) => key).join(', ')}`);
}

function mastered(scenario: CreditCycleScenario, attempt: AttemptState | undefined) {
  return Boolean(attempt?.revealed && attempt.choice === scenario.correct);
}

function statusOf(scenario: CreditCycleScenario, attempt: AttemptState | undefined) {
...
export default function CreditCycleLab() {
  const [attempts, setAttempts] = useState<Record<string, AttemptState>>({});
  const [mode, setMode] = useState<CreditCycleMode>(DEFAULT_MODE);
  ...
  const sequence = creditCycleScenarios;
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw && parsed) {
        ...
        notice = '本地记录未通过3.14 schema与题号白名单，已从空白进度继续。';
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(STORAGE_KEY, JSON.stringify({ schema: STORAGE_SCHEMA, attempts }));
    } catch {
      queueMicrotask(() => {
        setStorageState('session');
        setAnnouncement('浏览器拒绝本地保存；刷新可能丢失进度。');
      });
    }
  ...
  function navigate(nextMode: CreditCycleMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(choice: CreditCycleChoice) {
    setAttempts((current) => {
  ...
  function resetAll() {
    ...
    try {
      ...
      setAnnouncement('3.14十道题的本设备记录已清除，其他课程键未受影响。');
    } catch {
      setStorageState('session');
  ...
  return <div className={`impact-lab bank-credit-lab risk-taking-quiz ${styles.quiz}`}>
    <div className="impact-lab-head"><div><span>CREDIT CYCLE · ACCOUNTING, CONSTRAINT & IDENTIFICATION LAB</span><h3>从存量—流量账本一路检查到杠杆、抵押、偿债、资本反馈与识别边界</h3></div><p>M1–M9使用明确标记的SYNTHETIC参数，M10只解释冻结的BIS官方观测；所有计算另有K1–K10静态迁移题。</p></div>
    <noscript><style>{`.${styles.quiz} > :not(.impact-lab-head):not(noscript):not(#credit-cycle-static-twins){display:none!important}`}</style><div className="precision-note"><span>互动题当前不可用</span><p>请前往<a href="#credit-cycle-static-twins">K1–K10静态迁移题</a>完成相同机制练习。</p></div></noscript>
    <div aria-busy={loading} className="bank-credit-script-ui">
      ...
      <section aria-labelledby={`credit-cycle-question-${scenario.id}`} className="impact-question"><span>{scenario.label} · 全局第 {globalNumber}/{sequence.length} 题 · {scenario.synthetic ? 'SYNTHETIC' : 'OFFICIAL DATA INTERPRETATION'}</span><h3 id={`credit-cycle-question-${scenario.id}`} ref={questionRef} tabIndex={-1}>{scenario.title}</h3><p>{scenario.brief}</p></section>
      ...
      <fieldset className="impact-choice-fieldset" disabled={loading || attempt.revealed}><legend>选择唯一最完整的答案</legend>{scenario.options.map((option, index) => <label className={attempt.choice === option.id ? 'selected' : ''} key={option.id}><input checked={attempt.choice === option.id} name={`credit-cycle-choice-${scenario.id}`} onChange={() => choose(option.id)} ref={index === 0 ? firstOptionRef : undefined} type="radio" /><i>{option.id.toUpperCase()}</i><span>{option.label}</span></label>)}</fieldset>
      <fieldset className="impact-choice-fieldset" disabled={loading || attempt.revealed}><legend>提交前记录置信度</legend>{confidenceLabels.map((item) => <label key={item.id}><input checked={attempt.confidence === item.id} name={`credit-cycle-confidence-${scenario.id}`} onChange={() => chooseConfidence(item.id)} type="radio" /><span>{item.label}</span></label>)}</fieldset>
      ...
      {attempt.revealed ? <div aria-label={`${scenario.id}作答结果：${isCorrect ? '正确' : '错误'}`} className={isCorrect ? 'impact-result correct' : 'impact-result'} ref={resultRef} role="region" tabIndex={-1}><b>{isCorrect ? '回答正确：已解锁复算、判断、机制与失败条件' : '回答错误：标准答案仍未展示'}</b><p>{selected?.diagnosis}</p>{isCorrect ? <><output aria-label="逐步复算" className="impact-calculation">{scenario.calculation}</output><p><b>判断：</b>{scenario.judgment}</p><p><b>机制解释：</b>{scenario.mechanismExplanation}</p><p><b>反事实 / 失败条件：</b>{scenario.counterfactualOrFailure}</p><p><b>学习记录：</b>首次答案 {attempt.firstChoice?.toUpperCase()} · 首次置信度 {confidenceLabels.find(({ id }) => id === attempt.firstConfidence)?.label ?? '未记录'} · 已提交 {attempt.submissions ?? 1} 次。</p><p><b>复习入口：</b><a href={`#${scenario.primarySectionId}`}>主机制</a>{scenario.remediationSectionIds.map((id) => <span key={id}> · <a href={`#${id}`}>{id}</a></span>)}</p><SourceLinks ids={scenario.sourceIds} /><div>{nextUnmastered ? <button className="impact-secondary" onClick={goToNext} type="button">前往下一道未掌握题</button> : <strong>十题均已答对；请继续K1–K10静态迁移。</strong>}</div></> : <><p>请检查统计对象、存量—流量、分母、vintage以及预测/因果身份，再重做。</p><button className="impact-secondary" onClick={retry} type="button">保留首次记录并重做</button></>}</div> : null}
      <div aria-label="3.14本地作答记录控制" className="impact-result" role="group">{!resetArmed ? <button className="impact-secondary" disabled={loading} onClick={() => { pendingFocus.current = 'reset-confirm'; setResetArmed(true); setAnnouncement('尚未清除；再次确认才会删除3.14记录。'); }} ref={resetRequestRef} type="button">准备重置本实验</button> : <><button className="impact-secondary" onClick={() => { pendingFocus.current = 'reset-request'; setResetArmed(false); }} type="button">取消</button><button className="impact-secondary" onClick={resetAll} ref={resetConfirmRef} type="button">确认清除3.14十道题记录</button></>}</div>
      <div className="yield-fixture-audit" role="group" aria-label="3.14题库结构、数值与存储断言"><span>结构 {creditCycleScenarioAssertions.filter(({ passed }) => passed).length}/{creditCycleScenarioAssertions.length} · 数值 {creditCycleNumericAssertionAudit.filter(({ passed }) => passed).length}/{creditCycleNumericAssertionAudit.length} · 存储 {creditCycleStorageAudit.filter(({ passed }) => passed).length}/{creditCycleStorageAudit.length}</span><ul>{creditCycleScenarioAssertions.map((item) => <li className={item.passed ? 'passed' : ''} key={item.key}>{item.passed ? 'PASS' : 'FAIL'} · {item.key}</li>)}{creditCycleNumericAssertionAudit.map((item) => <li className={item.passed ? 'passed' : ''} key={`${item.scenarioId}.${item.key}`}>{item.passed ? 'PASS' : 'FAIL'} · {item.scenarioId}.{item.key}</li>)}{creditCycleStorageAudit.map((item) => <li className={item.passed ? 'passed' : ''} key={item.key}>{item.passed ? 'PASS' : 'FAIL'} · storage · {item.key}</li>)}</ul></div>
