...
import {
  bankLendingModes,
  bankLendingNumericAssertionAudit,
  bankLendingScenarioAssertions,
  bankLendingScenarios,
  type BankLendingChoice,
  type BankLendingMode,
  type BankLendingScenario,
} from './bankLendingScenarios';

type Confidence = 'low' | 'medium' | 'high';
type AttemptState = {
  choice?: BankLendingChoice;
  confidence?: Confidence;
  firstChoice?: BankLendingChoice;
  firstConfidence?: Confidence;
  submissions?: number;
  revealed?: boolean;
};

const STORAGE_KEY = 'market-world-model:3.10-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const DEFAULT_MODE = bankLendingModes[0].id;
const validChoices = new Set<BankLendingChoice>(['a', 'b', 'c']);
const validConfidence = new Set<Confidence>(['low', 'medium', 'high']);
const scenarioIds = new Set(bankLendingScenarios.map((scenario) => scenario.id));
...
function validateStored(value: unknown): { attempts: Record<string, AttemptState>; droppedInvalid: boolean } | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    if (!scenarioIds.has(id as BankLendingScenario['id']) || !raw || typeof raw !== 'object' || Array.isArray(raw)) {
      droppedInvalid = true;
      return;
    }
    const item = raw as Record<string, unknown>;
    const choice = item.choice as BankLendingChoice | undefined;
    const confidence = item.confidence as Confidence | undefined;
    const firstChoice = item.firstChoice as BankLendingChoice | undefined;
    ...
    const currentInvalid = revealed === true
      ? !validChoices.has(choice as BankLendingChoice) || !validConfidence.has(confidence as Confidence)
    ...
    const completeHistory = validChoices.has(firstChoice as BankLendingChoice)
...
function mastered(scenario: BankLendingScenario, attempt?: AttemptState) {
  return Boolean(attempt?.revealed && attempt.choice === scenario.correct);
}

function statusOf(scenario: BankLendingScenario, attempt?: AttemptState) {
...
export default function BankLendingLab() {
  const [mode, setMode] = useState<BankLendingMode>(DEFAULT_MODE);
  ...
  const sequence = bankLendingScenarios;
  ...
  const modeProgress = bankLendingModes.map((item) => ({
    ...item,
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw && parsed) {
        ...
        restoredNotice = parsed.droppedInvalid
          ? '无法验证的本地题库条目已丢弃；其余合法作答已恢复。'
          : Object.keys(restored).length > 0
            ? `已从本设备恢复 ${Object.keys(restored).length} 道题的作答记录。`
            : '';
      } else if (raw) {
        restoredNotice = '无法验证的本地题库内容已全部丢弃；已从空白进度继续。';
        try {
    ...
    queueMicrotask(() => {
      ...
      if (restoredNotice) setRecoveryNotice(restoredNotice);
  ...
  function navigate(nextMode: BankLendingMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(choice: BankLendingChoice) {
    setAttempts((current) => {
  ...
  function requestReset() {
    ...
    setResetArmed(true);
    setAnnouncement('重置尚未执行；再次确认才会清除3.10题库在本设备的记录。');
  }
  ...
  function resetAll() {
    ...
    try {
      window.localStorage.removeItem(STORAGE_KEY);
      setAnnouncement('3.10十道题的本地作答记录已重置；其他课程键未受影响。');
    } catch {
      setStorageState('session');
  ...
  return (
    <div className="impact-lab bank-credit-lab bank-lending-lab">
      <div className="impact-lab-head"><div><span>BANK LENDING · PRICING, ALLOCATION & IDENTIFICATION LAB</span><h3 ref={titleRef} tabIndex={-1}>先冻结借款人风险与需求，再判断银行成本、容量和合同如何改变贷款供给</h3></div><p>十道主题题与十道静态孪生均使用冻结合成参数；它们训练机制审计，不是贷款报价器、合规结论、政策预测或因果估计。</p></div>
      <noscript><style>{'.bank-lending-lab > :not(.impact-lab-head):not(noscript){display:none!important}'}</style><div className="precision-note"><span>互动实验当前不可用</span><p>无脚本时已隐藏不会随输入更新的控件与运行时断言；请前往<a href="#bank-lending-static-twins">十道静态孪生题</a>完成同一组机制练习。</p></div></noscript>
      <div aria-busy={loading} className="bank-credit-script-ui">
        ...
        <div className="impact-mode-picker" role="group" aria-label="选择银行贷款渠道题库模式">{bankLendingModes.map((item) => <button aria-pressed={mode === item.id} className={mode === item.id ? 'active' : ''} disabled={loading} key={item.id} onClick={() => navigate(item.id, 0)} type="button"><span>{item.label}</span><b>{item.title}</b><small>{item.description}</small></button>)}</div>
        ...
        {completed ? <div aria-label="实验完成诊断" className="impact-result correct" ref={resultRef} role="region" tabIndex={-1}><b>本轮完成：{modeProgress.map((item) => `${item.label} ${item.mastered}/${item.total}`).join('，')}</b><p>十题均已答对。复盘时先查看高置信首次答错题，再用 K1–K10 检查数值迁移，并始终分开pair估计量、借款人总融资的前后观察变化和实体结果的前后观察变化；三者只有在各自反事实闭合时才能获得相应因果标签。</p><button className="impact-secondary" onClick={() => navigate(DEFAULT_MODE, 0)} type="button">回到 M1 复盘</button></div> : <>
          <section className="impact-question" aria-labelledby={`bank-lending-question-${scenario.id}`}><span>{scenario.label} · 全局第 {globalNumber}/{sequence.length} 题 · SYNTHETIC</span><h3 id={`bank-lending-question-${scenario.id}`} ref={questionRef} tabIndex={-1}>{scenario.title}</h3><p>{scenario.brief}</p></section>
          ...
          {attempt.revealed ? <div aria-labelledby={resultTitleId} className={isCorrect ? 'impact-result correct' : 'impact-result'} ref={resultRef} role="region" tabIndex={-1}><b id={resultTitleId}>{isCorrect ? '回答正确：已解锁逐步复算' : '回答错误：尚未显示答案与复算'}</b><p>{selected?.diagnosis}</p>{isCorrect ? <><output aria-label="逐步复算" className="impact-calculation">{scenario.calculation}</output><strong>{scenario.reveal}</strong><p><b>学习记录：</b>首次答案 {attempt.firstChoice?.toUpperCase()} · 首次置信度 {confidenceLabels.find((item) => item.id === attempt.firstConfidence)?.label ?? '未记录'} · 已提交 {attempt.submissions ?? 1} 次。</p><p><b>复习入口：</b><a href={`#${scenario.primarySectionId}`}>主机制</a>{scenario.remediationSectionIds.map((id) => <span key={id}> · <a href={`#${id}`}>{id}</a></span>)}</p><SourceLinks ids={scenario.sourceIds} /><div><span aria-hidden="true" /><button className="impact-secondary" onClick={goToNext} type="button">{nextUnmastered ? '前往下一道未掌握题' : '查看完成诊断'}</button></div></> : <><p>请回到银行、借款人、合同、时钟和替代路径重新推导；错误状态不展示正确选项、完整计算、citation 或下一题。</p><div><button className="impact-secondary" onClick={retry} type="button">保留首次记录并重做</button></div></>}</div> : null}
        </>}

        <div aria-label="3.10本地作答记录控制" className="impact-result" role="group">{!resetArmed ? <button className="impact-secondary" disabled={loading} onClick={requestReset} ref={requestResetRef} type="button">准备重置本实验</button> : <><button className="impact-secondary" disabled={loading} onClick={() => { pendingFocus.current = 'reset-request'; setResetArmed(false); setAnnouncement('已取消重置，原记录保留。'); }} type="button">取消</button><button className="impact-secondary" disabled={loading} onClick={resetAll} ref={confirmResetRef} type="button">确认清除3.10十道题记录</button></>}</div>
        <div className="yield-fixture-audit" role="group" aria-label="题库结构断言"><span>题库结构断言 {bankLendingScenarioAssertions.filter((item) => item.passed).length}/{bankLendingScenarioAssertions.length}</span><ul>{bankLendingScenarioAssertions.map((item) => <li className={item.passed ? 'passed' : ''} key={item.id}>{item.passed ? '通过' : '失败'} · {item.statement}</li>)}</ul></div>
        <div className="yield-fixture-audit" role="group" aria-label="题库数值与边界断言"><span>题库数值与边界断言 {bankLendingNumericAssertionAudit.filter((item) => item.passed).length}/{bankLendingNumericAssertionAudit.length}</span><ul>{bankLendingNumericAssertionAudit.map((item) => <li className={item.passed ? 'passed' : ''} key={`${item.scenarioId}:${item.key}`}>{item.passed ? '通过' : '失败'} · {item.scenarioId}.{item.key}</li>)}</ul></div>
        <p className="impact-lab-caveat"><b>边界：</b>只有 <code>{STORAGE_KEY}</code> 保存通过白名单校验的 attempts。题号、答案、计算、来源、C1–C7 控件状态与其他课程输入不进入本地记录；客户端门控也不构成服务端防作弊。</p>
      </div>
    </div>
  );
}
