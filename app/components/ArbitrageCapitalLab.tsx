...
import {
  arbitrageCapitalModes,
  arbitrageCapitalScenarios,
  type ArbitrageCapitalChoice,
  type ArbitrageCapitalMode,
  type ArbitrageCapitalScenario,
} from './arbitrageCapitalScenarios';

type AttemptState = { choice?: ArbitrageCapitalChoice; revealed?: boolean };

const STORAGE_KEY = 'market-world-model:2.15-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<ArbitrageCapitalChoice>(['a', 'b', 'c']);
const scenarioIds = new Set(arbitrageCapitalScenarios.map((scenario) => scenario.id));
const sequence = arbitrageCapitalScenarios;
...
function validatedAttempts(value: unknown): Record<string, AttemptState> | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    if (!validChoices.has(attempt.choice as ArbitrageCapitalChoice) || typeof attempt.revealed !== 'boolean') return;
    result[id] = { choice: attempt.choice as ArbitrageCapitalChoice, revealed: attempt.revealed };
  });
  return result;
}

function taskStatus(scenario: ArbitrageCapitalScenario, attempt?: AttemptState) {
  if (!attempt?.revealed) return '未提交';
  return attempt.choice === scenario.correct ? '已答对' : '已提交，待重做';
}

function ScenarioFacts({ scenario }: { scenario: ArbitrageCapitalScenario }) {
  return (
...
export default function ArbitrageCapitalLab() {
  const [mode, setMode] = useState<ArbitrageCapitalMode>('object');
  ...
  const scenarios = arbitrageCapitalScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: ArbitrageCapitalMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function selectMode(nextMode: ArbitrageCapitalMode) {
  ...
  function choose(nextChoice: ArbitrageCapitalChoice) {
  ...
  function goToNextUnsubmitted() {
    if (nextUnsubmitted) {
      const nextModeScenarios = arbitrageCapitalScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  return (
    <div aria-busy={restoring} className="impact-lab bank-intermediary-lab arbitrage-capital-lab">
      <div className="impact-lab-head">
        <div><span>ARBITRAGE-CAPITAL SURVIVAL LAB</span><h3>从可执行价差到能否活到收敛</h3></div>
        <p>十题使用冻结教学参数。前五题先证明索赔等价并扣除成本、路径与借券；后五题把 haircut、资金流、共同减仓、两腿成交和 short-squeeze 证据放进真实资本账本；客户端包含答案，不作为防作弊考试。</p>
      </div>
      <noscript>
      ...
      <div className="impact-mode-picker" role="group" aria-label="实验模式">
        {arbitrageCapitalModes.map((item) => (
      ...
      <div className="impact-task-picker" role="group" aria-label={(mode === 'object' ? '识别可执行套利对象' : '审计套利资本与反馈') + '题目导航'}>
      ...
      {completed ? (
        <div className="impact-result" ref={resultRef} role="region" tabIndex={-1} aria-labelledby={resultTitleId}>
          ...
          <p>复习路径：第1题重建严格套利；第2–3题检查 claims equivalence 和 no-arbitrage band；第4–5题分开终点收益、路径亏损与 short carry；第6–8题把 haircut、delegated flows 和共同减仓变成资本反馈；第9题严格区分 target、order 与 fill；第10题训练 short squeeze 的多因果证据边界。错误题保留原选择，可逐题重做。</p>
          <div>
            ...
            {scenario.options.map((option, optionIndex) => (
              <label className={choice === option.id ? 'selected' : ''} key={option.id}>
                <input
                  ...
                  name={'arbitrage-capital-' + scenario.id}
      ...
      <p className="impact-lab-caveat"><b>模型边界：</b>所有 claims、prices、spreads、costs、haircuts、capital、flows、depth、positions 与 fills 都是为了得到唯一答案而冻结的教学参数，不代表任何证券、基金、broker 或市场的当前状态。线性 capacity 和 impact 只用于训练账本；题目不是投资建议、收益保证或真实交易；保存只限本设备、本浏览器和当前题库版本。</p>
    </div>
  );
}
