...
import {
  inflationModes,
  inflationScenarios,
  type InflationChoice,
  type InflationMode,
  type InflationScenario,
} from './inflationScenarios';

type Confidence = 'low' | 'medium' | 'high';
type AttemptState = {
  choice?: InflationChoice;
  confidence?: Confidence;
  firstChoice?: InflationChoice;
  firstConfidence?: Confidence;
  submissions?: number;
  revealed?: boolean;
};

const STORAGE_KEY = 'market-world-model:3.02-lab-r1';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<InflationChoice>(['a', 'b', 'c']);
const validConfidence = new Set<Confidence>(['low', 'medium', 'high']);
const scenarioIds = new Set(inflationScenarios.map((scenario) => scenario.id));
const sequence = inflationScenarios;
...
function validateStored(value: unknown): { attempts: Record<string, AttemptState>; droppedInvalid: boolean } | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    ...
    const choice = item.choice as InflationChoice | undefined;
    const confidence = item.confidence as Confidence | undefined;
    const firstChoice = item.firstChoice as InflationChoice | undefined;
    ...
    const currentInvalid = revealed === true
      ? !validChoices.has(choice as InflationChoice) || !validConfidence.has(confidence as Confidence)
    ...
    const completeHistory = validChoices.has(firstChoice as InflationChoice)
...
function statusOf(scenario: InflationScenario, attempt?: AttemptState) {
...
export default function InflationLab() {
  const [mode, setMode] = useState<InflationMode>('measurement');
  ...
  const scenarios = inflationScenarios.filter((item) => item.mode === mode);
  ...
  function navigate(nextMode: InflationMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(choice: InflationChoice) {
  ...
  function goToNext() {
    ...
    const nextModeScenarios = inflationScenarios.filter((item) => item.mode === nextUnsubmitted.mode);
  ...
  return (
    <div aria-busy={storageState === 'loading'} className="impact-lab inflation-lab">
      <div className="impact-lab-head">
        <div><span>INFLATION · MEASUREMENT-TO-PRICING LAB</span><h3 ref={titleRef} tabIndex={-1}>先固定指数对象与时钟，再把价格变化接到成本、加成、传导和公司毛利</h3></div>
        <p>前五题校准权重、增长时钟、CPI–PCE、质量住房与 breakeven；后五题复算成本加成、ULC、汇率传导、相对价持久性与毛利断裂。</p>
      </div>
      ...
      {completed ? (
        <div aria-label="实验完成诊断" className="impact-result correct" ref={resultRef} role="region" tabIndex={-1}>
          ...
          <p>完成不等于掌握。优先重做高置信却答错的题，再用静态孪生验证自己是否守住对象、单位、时钟、因果层级和公司断链。</p>
          <button className="impact-secondary" onClick={() => navigate('measurement', 0)} type="button">回到第一题逐题复盘</button>
        </div>
      ) : (
        <>
          <section className="impact-question" aria-labelledby={`inflation-question-${scenario.id}`}>
            <span>{scenario.label} · 全局第 {globalNumber}/10 题</span>
            <h3 id={`inflation-question-${scenario.id}`} ref={questionRef} tabIndex={-1}>{scenario.title}</h3>
            <p>{scenario.brief}</p>
          </section>
            ...
            {scenario.options.map((option, index) => (
              <label className={attempt.choice === option.id ? 'selected' : ''} key={option.id}>
                <input
                  ...
                  name={`inflation-choice-${scenario.id}`}
          ...
          <fieldset className="impact-choice-fieldset" disabled={storageState === 'loading' || attempt.revealed}>
            ...
            {confidenceLabels.map((item) => (
              <label key={item.id}>
                <input checked={attempt.confidence === item.id} name={`inflation-confidence-${scenario.id}`} onChange={() => chooseConfidence(item.id)} type="radio" value={item.id} />
      ...
      <p className="impact-lab-caveat"><b>边界：</b>题中权重、传导率、风险楔子、价格路径和公司账本只为唯一判分而冻结；它们不是通胀预测、盈利预测或投资建议。记录仅保存在本设备、本浏览器和当前题库版本，不会上传。</p>
    </div>
  );
}
