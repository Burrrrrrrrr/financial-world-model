...
import {
  realRateModes,
  realRateNumericAssertionAudit,
  realRateScenarioAssertions,
  realRateScenarios,
  type RealRateChoice,
  type RealRateMode,
  type RealRateScenario,
} from './realRateScenarios';

type Confidence = 'low' | 'medium' | 'high';
type AttemptState = {
  choice?: RealRateChoice;
  confidence?: Confidence;
  firstChoice?: RealRateChoice;
  firstConfidence?: Confidence;
  submissions?: number;
  revealed?: boolean;
};

const STORAGE_KEY = 'market-world-model:3.08-lab-r2';
const EMPTY_ATTEMPT: AttemptState = {};
const validChoices = new Set<RealRateChoice>(['a', 'b', 'c']);
const validConfidence = new Set<Confidence>(['low', 'medium', 'high']);
const scenarioIds = new Set(realRateScenarios.map((scenario) => scenario.id));
...
function validateStored(value: unknown): { attempts: Record<string, AttemptState>; droppedInvalid: boolean } | null {
  ...
  Object.entries(source.attempts as Record<string, unknown>).forEach(([id, raw]) => {
    if (!scenarioIds.has(id as RealRateScenario['id']) || !raw || typeof raw !== 'object' || Array.isArray(raw)) {
      droppedInvalid = true;
      return;
    }
    const item = raw as Record<string, unknown>;
    const choice = item.choice as RealRateChoice | undefined;
    const confidence = item.confidence as Confidence | undefined;
    const firstChoice = item.firstChoice as RealRateChoice | undefined;
    ...
    const currentInvalid = revealed === true
      ? !validChoices.has(choice as RealRateChoice) || !validConfidence.has(confidence as Confidence)
    ...
    const completeHistory = validChoices.has(firstChoice as RealRateChoice)
...
function mastered(scenario: RealRateScenario, attempt?: AttemptState) {
  return Boolean(attempt?.revealed && attempt.choice === scenario.correct);
}

function statusOf(scenario: RealRateScenario, attempt?: AttemptState) {
...
export default function RealRateLab() {
  const [mode, setMode] = useState<RealRateMode>('purchasing-power');
  ...
  const sequence = realRateScenarios;
  ...
  const purchasingPowerMastered = sequence.filter((item) => item.mode === 'purchasing-power' && mastered(item, attempts[item.id])).length;
  const valuationMastered = sequence.filter((item) => item.mode === 'valuation' && mastered(item, attempts[item.id])).length;
  ...
  function navigate(nextMode: RealRateMode, nextIndex: number, focus: 'question' | 'option' = 'question') {
  ...
  function choose(choice: RealRateChoice) {
    setAttempts((current) => {
  ...
  function requestReset() {
    ...
    setResetArmed(true);
    setAnnouncement('重置尚未执行；再次确认才会清除 3.08 题库在本设备的记录。');
  }

  function resetAll() {
    if (loading) return;
    setAttempts({});
    setMode('purchasing-power');
    setScenarioIndex(0);
    setCompleted(false);
    setResetArmed(false);
    setRecoveryNotice('');
    ...
    try {
      ...
      setAnnouncement('3.08 十道题的本地作答记录已重置；其他课程键未受影响。');
    } catch {
      setStorageState('session');
  ...
  return (
    <div aria-busy={loading} className="impact-lab real-rate-lab">
      <div className="impact-lab-head"><div><span>REAL RATES · PURCHASING POWER & VALUATION LAB</span><h3 ref={titleRef} tabIndex={-1}>先给实际利率办“对象护照”，再决定它能否进入某类索赔权的贴现率</h3></div><p>十道主题和十道静态孪生均使用冻结合成值；不包含当前市场利率或投资建议。</p></div>
      <noscript><div className="precision-note"><span>互动实验当前不可用</span><p>此实验需要 JavaScript；你仍可前往<a href="#real-rate-static-twins">十道 SSR 静态孪生题</a>完成同一组机制练习。</p></div></noscript>
      {recoveryNotice ? <div className="precision-note" role="status"><span>本地记录已清理</span><p>{recoveryNotice}</p></div> : null}
      <p aria-live="polite" className="impact-lab-progress">当前已答对 {masteredCount}/{sequence.length} · 购买力 {purchasingPowerMastered}/5 · 估值 {valuationMastered}/5 · 已提交 {submittedCount}/{sequence.length} · {storageState === 'saved' ? '本设备自动保存' : storageState === 'session' ? '仅当前会话' : '正在恢复记录'}</p>
      <p aria-live="polite" className="sr-only">{announcement}</p>

      <div className="impact-mode-picker" role="group" aria-label="选择实际利率题库模式">{realRateModes.map((item) => <button aria-pressed={mode === item.id} className={mode === item.id ? 'active' : ''} disabled={loading} key={item.id} onClick={() => navigate(item.id, 0)} type="button"><span>{item.label}</span><b>{item.title}</b><small>{item.description}</small></button>)}</div>
      ...
      {completed ? <div aria-label="实验完成诊断" className="impact-result correct" ref={resultRef} role="region" tabIndex={-1}><b>本轮完成：购买力 {purchasingPowerMastered}/5，估值 {valuationMastered}/5</b><p>十题均已答对。复盘时优先检查“高置信首次答错”的题，再用 K1–K10 静态孪生检查迁移。</p><button className="impact-secondary" onClick={() => navigate('purchasing-power', 0)} type="button">回到 M1 复盘</button></div> : <>
        <section className="impact-question" aria-labelledby={`real-rate-question-${scenario.id}`}><span>{scenario.label} · 全局第 {globalNumber}/{sequence.length} 题 · SYNTHETIC</span><h3 id={`real-rate-question-${scenario.id}`} ref={questionRef} tabIndex={-1}>{scenario.title}</h3><p>{scenario.brief}</p></section>
        ...
        {attempt.revealed ? <div aria-labelledby={resultTitleId} className={isCorrect ? 'impact-result correct' : 'impact-result'} ref={resultRef} role="region" tabIndex={-1}><b id={resultTitleId}>{isCorrect ? '回答正确：已解锁逐步复算' : '回答错误：尚未显示答案与复算'}</b><p>{selected?.diagnosis}</p>{isCorrect ? <><output aria-label="逐步复算" className="impact-calculation">{scenario.calculation}</output><strong>{scenario.reveal}</strong><p><b>学习记录：</b>首次答案 {attempt.firstChoice?.toUpperCase()} · 首次置信度 {confidenceLabels.find((item) => item.id === attempt.firstConfidence)?.label ?? '未记录'} · 已提交 {attempt.submissions ?? 1} 次。</p><p><b>复习入口：</b><a href={`#${scenario.primarySectionId}`}>主机制</a>{scenario.remediationSectionIds.map((id) => <span key={id}> · <a href={`#${id}`}>{id}</a></span>)}</p><SourceLinks ids={scenario.sourceIds} /><div><span aria-hidden="true" /><button className="impact-secondary" onClick={goToNext} type="button">{nextUnmastered ? '前往下一道未掌握题' : '查看完成诊断'}</button></div></> : <><p>请回到冻结参数、单位、价格指数与索赔权边界重新判断；错误状态不展示正确选项、完整计算、citation 或下一题。</p><div><button className="impact-secondary" onClick={retry} type="button">保留首次记录并重做</button></div></>}</div> : null}
      </>}

      <div aria-label="3.08 本地作答记录控制" className="impact-result" role="group">{!resetArmed ? <button className="impact-secondary" disabled={loading} onClick={requestReset} ref={requestResetRef} type="button">准备重置本实验</button> : <><button className="impact-secondary" disabled={loading} onClick={() => { pendingFocus.current = 'reset-request'; setResetArmed(false); setAnnouncement('已取消重置，原记录保留。'); }} type="button">取消</button><button className="impact-secondary" disabled={loading} onClick={resetAll} ref={confirmResetRef} type="button">确认清除 3.08 十道题记录</button></>}</div>
      <div className="yield-fixture-audit" role="group" aria-label="题库结构断言"><span>题库结构断言 {realRateScenarioAssertions.filter((item) => item.passed).length}/{realRateScenarioAssertions.length}</span><ul>{realRateScenarioAssertions.map((item) => <li className={item.passed ? 'passed' : ''} key={item.id}>{item.passed ? '通过' : '失败'} · {item.statement}</li>)}</ul></div>
      <div className="yield-fixture-audit" role="group" aria-label="题库全部数值断言"><span>题库数值逐项复算 {realRateNumericAssertionAudit.filter((item) => item.passed).length}/{realRateNumericAssertionAudit.length}</span><ul>{realRateNumericAssertionAudit.map((item) => <li className={item.passed ? 'passed' : ''} key={`${item.scenarioId}:${item.key}`}>{item.passed ? '通过' : '失败'} · {item.scenarioId}.{item.key}</li>)}</ul></div>
      <p className="impact-lab-caveat"><b>边界：</b>只有 <code>{STORAGE_KEY}</code> 保存通过白名单校验的 attempts。模式、题号、得分、正确答案、完整计算、来源、C1–C7 控件状态与其他课程输入都不进入本地记录。</p>
    </div>
  );
}
